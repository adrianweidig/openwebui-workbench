...
- Modellprofile werden über `scripts/configure_openwebui_tool_models.py` vereinheitlicht: natives Tool-Calling, eingebettete Icons, use-case-spezifische `temperature`-/`top_p`-Werte und High-Reasoning-Anweisung im Systemprompt. `max_tokens` wird bewusst nicht gesetzt; die Zielinstanz nutzt ihre Kontext- und Antwortlimits.
