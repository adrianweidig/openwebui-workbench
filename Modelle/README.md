...
- Modellprofile werden über `scripts/configure_openwebui_tool_models.py` vereinheitlicht: natives Tool-Calling, Vision-Fähigkeit für Mistral-Medium-VL, eingebettete Icons, use-case-spezifische `temperature`-/`top_p`-Werte, High-Reasoning-Anweisung, Beispielwissen und verbindliche Tool-Nutzungsregeln im Systemprompt. `max_tokens` wird bewusst nicht gesetzt; die Zielinstanz nutzt ihre Kontext- und Antwortlimits.
