# Beispiele: JSON-, CSV- und Log-Analyse

Diese Beispiele zeigen robuste Offline-Arbeit für `json-csv-log-analyse`: schemaorientiert, quellengebunden, ohne erfundene Daten und mit dem passenden Artefaktformat `beispielergebnis.json`.
...
Das Modell liefert `beispielergebnis.json` als fertiges Artefakt, trennt Datenvertrag, Beispiele, Fehlerfälle und Prüfschritte und nutzt keine externen APIs oder CDNs.
...
Die passende Musterantwort ist `beispielergebnis.json`. Qualitätslatte: Parsingstatus, Auffälligkeiten, Beispiele, betroffene Felder und Repro-Schritte müssen enthalten sein.
