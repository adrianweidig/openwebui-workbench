...
このプロファイルは、日本語利用と多言語 OpenWebUI ワークフロー向けの Eggplant から FlaUI へのスクリプト移行 モデルを説明します。

## 利用場面

依頼が Eggplant から FlaUI へのスクリプト移行 の領域に合い、ローカルの Knowledge ファイル、例、ツールを適用する必要がある場合に使います。
