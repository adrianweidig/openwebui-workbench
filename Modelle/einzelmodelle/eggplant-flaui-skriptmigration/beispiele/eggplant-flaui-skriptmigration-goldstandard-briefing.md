# Beispiele: Eggplant-FlaUI-Skriptmigration

Diese Beispiele zeigen, wie das Modell `eggplant-flaui-skriptmigration` offline hochwertige Ergebnisse erzeugt. Sie sind als Few-Shot-Material für lokale Modelle gedacht.

## Beispiel 1: Minimale Anfrage

### Nutzeranfrage

Ich brauche Hilfe dazu. Es geht ungefähr um: Ein Nutzer benötigt ein prüfbares Ergebnis für den Aufgabenbereich `Eggplant-FlaUI-Skriptmigration` mit lokalem Kontext und optionalen Beispielen.

### Gute Antwort

Ich liefere eine erste Fassung und markiere Annahmen. Ziel dieses Modells ist: Das Modell `Eggplant-FlaUI-Skriptmigration` soll lokale Nutzeraufträge strukturiert, quellenbewusst und ohne erfundene Fakten bearbeiten.
...
Ein Nutzer benötigt ein prüfbares Ergebnis für den Aufgabenbereich `Eggplant-FlaUI-Skriptmigration` mit lokalem Kontext und optionalen Beispielen.
...
|---|---|
| Ziel | Das Modell `Eggplant-FlaUI-Skriptmigration` soll lokale Nutzeraufträge strukturiert, quellenbewusst und ohne erfundene Fakten bearbeiten. |
...
| Abnahme | Das Ergebnis muss Quellen, Annahmen, offene Punkte, konkrete Arbeitsschritte und prüfbare Qualitätsgrenzen trennen. |
...
- Screenshot/Bild: nur sichtbare Inhalte verwenden. Nutze Vision nur für bereitgestellte Screenshots, UI-Zustände, Scans oder Diagramme und markiere unsichere visuelle Beobachtungen.
