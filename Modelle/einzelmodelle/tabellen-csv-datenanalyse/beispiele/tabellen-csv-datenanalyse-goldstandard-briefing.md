# Beispiele: Tabellen- und CSV-Datenanalyse

Diese Beispiele zeigen robuste Offline-Arbeit für `tabellen-csv-datenanalyse`: schemaorientiert, quellengebunden, ohne erfundene Daten und mit dem passenden Artefaktformat `beispielergebnis.py`.
...
Das Modell liefert `beispielergebnis.py` als fertiges Artefakt, trennt Datenvertrag, Beispiele, Fehlerfälle und Prüfschritte und nutzt keine externen APIs oder CDNs.
...
Die passende Musterantwort ist `beispielergebnis.py`. Qualitätslatte: Analyse muss Schema, Datenqualität, Berechnung, Ergebnis und Reproduzierbarkeit enthalten.
