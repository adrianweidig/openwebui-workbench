...
class Tools:
    class Valves(BaseModel):
        ...
        fallback_api_key: str = Field(
            default="",
            description="Disabled for Air-Gap. External fallback APIs are intentionally not used.",
        )
        fallback_base_url: str = Field(
            default="",
            description="Disabled for Air-Gap. Leave empty; use the local OpenWebUI API instead.",
        )
        council_models: str = Field(
            default=DEFAULT_COUNCIL_MODELS,
            description="Comma-separated local model IDs (default: coder) or 'all' to use local OpenWebUI models only.",
        )
    ...
    def _resolve_fallback_api_key(self) -> Optional[str]:
        """
        Air-gap build: public fallback APIs are disabled.
        """
        return None
    ...
    def _try_fallback(self) -> Tuple[Optional[str], Optional[str]]:
        """
        Air-gap build: public fallback APIs are disabled.
        Returns (None, None) so only the local OpenWebUI API is used.
        """
        return None, None
        ...
        if not api_key:
            ...
            if fallback_key:
                ...
                await self._emit_status(
                    __event_emitter__,
                    "error",
                    "No local OpenWebUI API key found. Set OPENWEBUI_API_KEY or configure openwebui_api_key in Valves.",
                    True,
                )
                return "Error: No local OpenWebUI API key found. Please set OPENWEBUI_API_KEY or configure openwebui_api_key in Valves."
