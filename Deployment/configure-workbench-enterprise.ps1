param(
    ...
    [string]$PortainerUrl = "",
    [ValidateSet("", "bundled", "bind")]
    [string]$WorkbenchWorkspaceMode = "",
...
function Write-TextFile {
    ...
    [System.IO.File]::WriteAllText($PathValue, (($Lines -join [Environment]::NewLine) + [Environment]::NewLine), $utf8NoBom)
}
...
if ([string]::IsNullOrWhiteSpace($WorkbenchWorkspaceMode)) {
    if ($NonInteractive) {
        $WorkbenchWorkspaceMode = "bundled"
    }
    else {
        $workspaceMode = Read-WorkbenchValue -Prompt "Workbench-Workspace aus Image-Volume oder Host-Repository verwenden? (bundled/bind)" -Default "bundled" -Required
        if ($workspaceMode -notin @("bundled", "bind")) {
            throw "Erlaubte Werte: bundled oder bind."
        }
        $WorkbenchWorkspaceMode = $workspaceMode
    }
}

if ($WorkbenchWorkspaceMode -eq "bind") {
...
$envLines = @(
    ...
    "WORKBENCH_WORKSPACE_MODE=$WorkbenchWorkspaceMode",
...
if ($OpenWebUIMode -eq "bundled") {
    ...
    if ($WorkbenchWorkspaceMode -eq "bind") {
        ...
        $composeLines += @(
            "    volumes:",
            "      - openwebui-data:/app/backend/data",
            "      - workbench-workspace:/app/backend/data/openwebui-workbench:ro"
        )
    }
    if ($RootCaPath) {
        $composeLines += @(
...
if ($OpenWebUIMode -eq "bundled" -or $WorkbenchWorkspaceMode -eq "bundled") {
    $composeLines += "volumes:"
    if ($OpenWebUIMode -eq "bundled") {
        $composeLines += "  openwebui-data:"
    }
    if ($WorkbenchWorkspaceMode -eq "bundled") {
        $composeLines += "  workbench-workspace:"
    }
    $composeLines += ""
}
