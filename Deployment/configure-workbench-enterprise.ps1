...
if ($OpenWebUIMode -eq "bundled") {
    ...
    $OpenWebUIPublicUrl = Read-WorkbenchValue -Prompt "Browser-URL fuer OpenWebUI" -Default $(if ($OpenWebUIPublicUrl) { $OpenWebUIPublicUrl } else { $OpenWebUIBaseUrl }) -Required
}
...
$authPassword = Read-WorkbenchSecret -Prompt "Workbench-Passwort (leer lassen, wenn spaeter in Portainer gesetzt wird)"
$adminToken = Read-WorkbenchSecret -Prompt "OpenWebUI-Admin-Token fuer Sync-Aktionen (optional)"

$envPath = Join-Path $OutputDir "workbench.env"
$composePath = Join-Path $OutputDir "portainer-compose.yml"
$openwebuiSecret = Read-WorkbenchSecret -Prompt "WEBUI_SECRET_KEY fuer gebuendeltes OpenWebUI (optional)"
